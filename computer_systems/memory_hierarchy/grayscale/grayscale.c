...
void grayscale(unsigned char *pixels, int32_t width, int32_t height) {
  ...
  // x is COLUMN
  // y is ROW
  // pixels are laid out in ROW order. Row is brought in cache line
  // by going on row first (y), we bring the whole row into the cache
  // then we go through each column (x) of that row (y)
  for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++) {
      offset = 3 * (y * width + x);
