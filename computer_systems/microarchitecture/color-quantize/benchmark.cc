...
extern "C" unsigned char quantize(unsigned char red, unsigned char green,
                                  unsigned char blue);
extern "C" unsigned char quantize_prior(unsigned char red, unsigned char green,
                                        unsigned char blue);
...
static void BM_quantize_prior(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < 3 * NUM_PIXELS; i += 3) {
      benchmark::DoNotOptimize(
          quantize_prior(pixels[i], pixels[i + 1], pixels[i + 2]));
    }
  }
}

BENCHMARK(BM_quantize_prior)
